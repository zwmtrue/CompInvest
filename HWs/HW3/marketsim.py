...
class order:
    ...
    def  __init__(self, orderinfo ):
        ...
        self.date =  dt.datetime(int(info[0]),int(info[1]),\
        int(info[2]),16,00)
        self.symbol = info[3]
    ...
    def __str__(self):
        s = str(self.date)+'   '+ self.exchange +\
        '  '+self.symbol +'  in quantity of  '+ str(abs(self.volume))
        return s
...
#step 3
    df_trade = pd.DataFrame(init_val,index = ls_alldates,\
    columns = ls_symbols)
    ordercount = 0
    for one_order in ls_orders:
        try:
            ...
            alreadytrade = 0
        df_trade.set_value(one_order.date, one_order.symbol, \
        one_order.volume + alreadytrade )
#step 4
